...
如果采用真空面起射，起始角通量本来就是精确的零入流边界值，因此通常不需要再额外设置 `distance_inactive`；更常见的做法是直接令它为 `0`。
...
- 追踪层面：
  - 真空面起射的射线：如果另一侧没有非 void cell，就认为发生泄漏并终止射线
  - 体内随机起射的射线：如果另一侧没有非 void cell，程序会沿当前边界法向做一次镜像回折，把全部能群的角通量重置为 `0`，再继续推进剩余路径长度
...
这里的“镜像回折 + `psi=0`”只是一种数值延拓技巧，不是把真空面改成真实反射边界。它的作用是让体内随机起射的有限长射线，也能在后续路径里感受到“边界入流为零”的条件；否则在均匀真空 slab 这类问题里，内部起射射线会一直停留在 `Q / Sigma_t` 的局部平衡角通量上，直到出界才结束，结果就会退化到接近无泄漏的 `k_inf` 解。
...
- `ray_launch_mode=volume`
  - 始终体内随机起射
  - 射线撞上真空/void 泄漏面后，会镜像回域内并把角通量重置为零，再继续推进剩余路径长度
- `ray_launch_mode=vacuum-surface`
  - 始终要求真空面起射
  - 如果几何里没有与 `ray_source` 盒子重合的 `x/y/z-plane` 真空外边界，程序会直接报错
  - 一般推荐把 `distance_inactive` 设为 `0`，因为真空面起射不需要再靠 dead zone 消除错误初值
